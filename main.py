...
DIAG_QUESTIONS = [
    ...
    ("company_niche", "2️⃣ В какой нише развивается компания (2–3 предложения)?"),
    ("company_age", "3️⃣ Сколько лет бизнесу?"),

    # ШАГ 2 — Продукт
    ("main_product", "1️⃣ Что является основным продуктом или услугой? Опиши простыми словами: что вы продаёте и какую задачу это решает."),
    ("product_value", "2️⃣ В чём ключевая ценность продукта? Почему клиенту стоит выбрать вас?"),
    ("product_strengths", "3️⃣ Какие три сильные стороны продукта? (качество, скорость, гарантия, сервис и т.п.)"),
    ("product_weaknesses", "4️⃣ Какие слабые стороны или ограничения продукта? Что мешает продавать больше?"),
    ("product_inside", "5️⃣ Как выглядит продукт «изнутри»? (состав, функции, процессы, технология, форматы)"),
    ("product_diff", "6️⃣ Чем ваш продукт отличается от конкурентов? 1–2 объективных отличия."),
    ("product_improve", "7️⃣ Что хотите улучшить или изменить в продукте в ближайшие 3 месяца?"),

    # ШАГ 3 — Целевая аудитория
    ("target_main", "1️⃣ Кто ваш основной клиент? (1 предложение)"),
    ("target_need", "2️⃣ Какую задачу или потребность клиент закрывает вашим продуктом?"),
    ("target_why_you", "3️⃣ Почему клиент выбирает вас, а не альтернативу? (1–2 причины)"),
    ("target_doubts", "4️⃣ Какие сомнения чаще всего возникают у клиента перед покупкой?"),
    ("target_factors", "5️⃣ Какие три фактора сильнее всего влияют на решение купить?"),
    ("target_sources", "6️⃣ Где аудитория ищет информацию и принимает решение?"),
    ("target_not", "7️⃣ Кто НЕ является вашим клиентом?"),

    # ШАГ 4 — Каналы трафика
    ("traffic_channels", "1️⃣ Какие каналы привлечения клиентов вы используете сейчас?"),
    ("traffic_main", "2️⃣ Какие каналы дают основной поток заявок или продаж?"),
    ("traffic_analytics", "3️⃣ Вы ведёте аналитику?"),
    ("traffic_budget", "4️⃣ Какой рекламный бюджет в месяц?"),
    ("traffic_improve", "5️⃣ Что хотите улучшить в системе привлечения клиентов?"),
    ("traffic_team", "6️⃣ Есть ли команда для продвижения? Штат или подрядчики?"),

    # ШАГ 5 — Команда (если нужно оставить блок пустым — скажи)
    ("team_info", "Команда: опишите структуру, роли, зоны ответственности (если есть).")
]
